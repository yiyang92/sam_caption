...
class AttModel():
    ...
    def __img_embeddings(self):
        ...
        if self._n_comms > 1:
            ...
            # [batch_size*5, 7 x 7, resnet.final_size]
            im_features = tf.tile(
                tf.expand_dims(im_features, 1), [1, self._n_comms, 1, 1])
            im_features = tf.reshape(
                im_features, 
                [self._batch_size * self._n_comms, 7 * 7, self._img_embed_size])
        self._im_features = im_features

    def __initial_gru(self, features):
        with tf.variable_scope('initial_gru'):
            features_mean = tf.reduce_mean(features, 1)
            h = tf.nn.tanh(tf.layers.dense(features_mean, self._com_hid_dim))
            return h
    ...
    def __build_layers(self):
        ...
        comm_decoder = Decoder(
            "comments_dec", self._com_hid_dim, self._embed_dim, comm_vocab, 0.9)
        dec_state = None
    ...
    def __build_inference_graph(self):
        ...
        attr_att = self.__attention_layer(
            self._im_features, self._state, "1")
